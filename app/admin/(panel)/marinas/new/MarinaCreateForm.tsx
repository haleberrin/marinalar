...
type RelationOption = {
  ...
  interface MarinaCreateFormProps {
...
export default function MarinaCreateForm({
    regions,
    cities,
    districts,
    categories,
  ...
  async function handleSubmit(
    ...
    if (!response.ok) {
      setError(data.error ?? "Marina oluşturulamadı.");
      return;
    }
  ...
  return (
    <form
      ...
      className="space-y-8 rounded-[28px] border border-slate-200 bg-white p-8 shadow-sm"
    >
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="md:col-span-2">
          ...
          <textarea
            ...
            required
            rows={6}
            placeholder="Her paragrafı yeni satıra yazabilirsiniz."
            className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-primary"
          />
        </div>
        ...
        <Input
          name="coverImage"
          label="Kapak Görseli"
          placeholder="/images/marinas/ornek.jpg"
          required
        />
...
/>
      ...
      <div className="flex justify-end">
        <button
          type="submit"
          disabled={loading}
          className="rounded-2xl bg-primary px-6 py-3 font-semibold text-white disabled:opacity-50"
        >
          {loading
            ? "Kaydediliyor..."
            : "Marinayı Kaydet"}
